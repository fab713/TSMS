...
<SCRIPT type="text/javascript">
	...
	$(function() {//预加载方法
		...
		$('#tabs').tabs('add', {
			title : '欢迎使用' ,
			content : createFrame('${baseurl}index/welocome.action')
		}).tabs({
	...
	function logout() {
		_confirm('您确定要退出本系统吗?',null,
				function(){
					location.href = '${baseurl}/login/logout.action';
				}
		)
	}
	...
	function showTime(){
		var date=new Date();
		$('#timeInfo').html();
		$('#timeInfo').html('&nbsp;&nbsp;&nbsp;&nbsp'+date.toLocaleString()+'&nbsp;&nbsp');
	}
...
<BODY style="overflow-y: hidden;" class="easyui-layout" scroll="no" >
	<DIV
		...
		border="false" split="true" region="north" onload="showTime();">
		<SPAN style="padding-right: 20px; float: right;" class="head">
			...
			TSMS</SPAN> <SPAN style="padding-left: 15px;" id="News"></SPAN>
	</DIV>
